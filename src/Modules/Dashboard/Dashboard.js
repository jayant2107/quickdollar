...
const Dashboard = () => {
  ...
  const [cardTotalUser, setCardTotalUser] = useState({});
  const [cardActiveOffer, setCardActiveOffer] = useState({});
  const [cardCompletedOffer, setCardCompletedOffer] = useState({});
...
C491.375,120.986,496,130.003,496,140.117z"
  ...
  //   CompletedOffersmonth: cardInfo?.CompletedOffersmonths?.map((offer, index) => {
